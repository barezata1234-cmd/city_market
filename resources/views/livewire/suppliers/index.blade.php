<div>
    <div class="flex items-center justify-between mb-5">
        <h1 class="text-xl font-bold text-gray-700">🚚 دابینکەران</h1>
        <button wire:click="openCreate" class="bg-[#141a2b] text-white px-4 py-2 rounded-lg text-sm">+ زیادکردنی دابینکەر</button>
    </div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr class="border-t">
                    ...
                    <td>{{ $s->phone }}</td>
                    <td>{{ $s->email }}</td>
                    <td class="{{ $s->balance > 0 ? 'text-red-500 font-semibold' : '' }}">{{ number_format($s->balance, 0) }} د.ع</td>
                    <td class="space-x-2 space-x-reverse">
                        <button wire:click="edit({{ $s->id }})" class="text-blue-600 text-xs">دەستکاری</button>
                        <button wire:click="delete({{ $s->id }})" wire:confirm="دڵنیایت؟" class="text-red-600 text-xs">سڕینەوە</button>
                    </td>
                </tr>
                @empty
                <tr><td colspan="5" class="text-center text-gray-400 py-8">هیچ دابینکەرێک نییە</td></tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" wire:click.self="$set('showModal', false)">
        <div class="bg-white rounded-xl p-6 w-full max-w-md">
            <h2 class="font-bold text-lg mb-4">{{ $editingId ? 'دەستکاریکردن' : 'زیادکردنی دابینکەری نوێ' }}</h2>
            <div class="space-y-3">
                <div>
